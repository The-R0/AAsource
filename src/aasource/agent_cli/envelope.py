...
from aasource.domain.enums import EnvelopeStatus
from aasource.domain.errors import AshareDataError, exit_code_for
from aasource.domain.models import Envelope, SourceRef, WarningItem
...
def fail(
    ...
    *,
    ...
    if isinstance(error, AshareDataError):
        ...
        err = AshareDataError(
            code=__import__("aasource.domain.errors", fromlist=["ErrorCode"]).ErrorCode.INTERNAL_ERROR,
            message=str(error),
            retryable=False,
        )
        code = 6
