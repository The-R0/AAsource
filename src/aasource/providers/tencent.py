...
from aasource.domain.identifiers import canonicalize_symbol, tencent_symbol
...
def parse_tencent_quotes(text: str) -> list[dict[str, Any]]:
    ...
    for record in text.split(";"):
        if '="' not in record:
            continue
        header = record.split('="', 1)[0].strip()
        vendor_match = re.search(r"v_((?:sh|sz|bj)\d{6})$", header, re.IGNORECASE)
        ...
        quotes.append(
            {
                "code": cells[2],
                "symbol": canonicalize_symbol(vendor_match.group(1)) if vendor_match else None,
                "name": cells[1],
